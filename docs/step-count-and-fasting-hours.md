...
`sleep_hours` は DB には直接保存しない。算出に使う 2 つの時刻フィールドから自動導出される。

```
daily_logs.bed_time      (TIME, nullable)  # 就寝時刻（HH:MM）
daily_logs.weigh_in_time (TIME, nullable)  # 体重測定時刻（HH:MM）— 空腹時間と共有
daily_logs.sleep_hours   (REAL, nullable)  # 導出値。直接書き込まない
```

- `bed_time` と `weigh_in_time` はいずれも未入力なら `NULL`
- `sleep_hours` は保存アクション（`save_daily_log_partial` RPC）が `bed_time` と `weigh_in_time` から自動算出して書き込む
- どちらか一方だけが更新された場合、もう一方を DB から取得したうえで再導出する（DB マージ方式）
...
| 推定睡眠時間の導出ロジック | `src/lib/utils/sleep.ts` (`deriveSleepHours`) |
