...
- 前日夜就寝（`bed_time > weigh_in_time`）として解釈される場合、保存基盤は sleep 系フィールドを翌日の起床日レコードへ振り分ける
...
- 新規入力は MealLogger の `bed_time` / `weigh_in_time` 入力 → 保存基盤での自動導出と保存先振り分けによって起床日基準に揃う
