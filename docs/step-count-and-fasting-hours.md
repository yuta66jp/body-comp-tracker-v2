...
> 旧モデル（`daily_logs.bed_time` → `deriveSleepHours()` → `daily_logs.sleep_hours`）は廃止方向。
...
daily_logs.bed_time     (TIME, nullable)  # 移行期カラム（廃止方向）。新規書き込み不可
```
...
- `daily_logs.bed_time` には過去データが残存しているが、移行期カラムとして読み取り専用扱い。廃止 migration は将来の別 Issue で実施予定
