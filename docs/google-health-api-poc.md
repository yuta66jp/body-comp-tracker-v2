...
|---|---|---|---|---|---|
| ステップ数 | `dailyRollUp` | `steps` | `googlehealth.activity_and_fitness.readonly` | `rollupDataPoints[].steps.countSum` | 日次集計値が必要なため `list` より `dailyRollUp` を優先する |
...
- Request body:
  - `range.start` / `range.end`: civil time の closed-open range
  - `windowSizeDays`: `1`
