...
本番向けの Google Cloud OAuth、server-only env、quota / billing、OAuth verification の確認項目は [Google Health 本番設定 / セキュリティ運用チェックリスト](google-health-production-readiness.md) を参照する。

## 取得対象

| 項目 | dataType | scope |
|---|---|---|
| 体重 | `weight` | `googlehealth.health_metrics_and_measurements.readonly` |
...
|---|---|---|---|---|---|
| 体重 | `list` | `weight` | `googlehealth.health_metrics_and_measurements.readonly` | `dataPoints[].weight.weightGrams` | kg に変換して `daily_logs.weight` に保存する。同日に複数ログがある場合は自動選択せずスキップする |
| ステップ数 | `reconcile` / `dailyRollUp` / `list` | `steps` | `googlehealth.activity_and_fitness.readonly` | `dataPoints[].steps.count` または `rollupDataPoints[].steps.countSum` | 複数ソースの重複を避けるため `reconcile` を優先する。失敗時は `dailyRollUp`、必要に応じて `list` で補完する |
...
体重は `weight` の `list` を使う。保存先は Google Health 日次メトリクステーブルではなく、既存の体重 source である `daily_logs.weight` とする。

- Endpoint: `GET https://health.googleapis.com/v4/users/me/dataTypes/weight/dataPoints`
- Filter: `weight.sample_time.civil_time >= "<startDate>" AND weight.sample_time.civil_time < "<endExclusiveDate>"`
- Response:
  - `weight.sampleTime.physicalTime`
  - `weight.sampleTime.civilTime`
  - `weight.weightGrams`

同期ルール:

- `weightGrams / 1000` を kg として保存する。
- 日付は `weight.sampleTime.civilTime.date` を優先し、取れない場合は `physicalTime` と `utcOffset` から補完する。
- 1 日 1 件だけ体重ログがある場合は、`daily_logs.weight` を作成または更新する。
- 日次ログが存在しない日でも、体重ログが 1 件なら `daily_logs` を作成する。
- 同日に複数の体重ログがある場合は、平均化・最新値採用をせず、その日をスキップする。スキップ理由は同期 API response とダッシュボードの同期結果に表示する。

### ステップ数

日次歩数は `steps` の `reconcile` を優先して使う。`reconcile` は複数データソースを単一ストリームへ統合するため、Fitbit Air とスマホ由来の歩数を単純合算しない。

- Endpoint: `GET https://health.googleapis.com/v4/users/me/dataTypes/steps/dataPoints:reconcile`
- Query:
...
`reconcile` が失敗した場合は `steps` の `dailyRollUp` を使う。
...
`dailyRollUp` が `400 Invalid argument in request.` を返す場合は、`steps` の `list` を最後のフォールバックとして使う。
...
`list` フォールバック時は `stepsResult.source` が `listFallback` になり、`stepsResult.fallbackFrom` に `dailyRollUp` のエラー情報が入る。`stepsResult.dataPoints` には raw response として Fitbit とスマホ由来のデータが混在する場合があるが、`dailyMetrics[].stepCount` は `dataSource.platform = "FITBIT"` の `steps.count` だけを日付ごとに合算した値を返す。
...
- `weightMetrics.metrics[]` に同期可能な体重ログ、`weightMetrics.skipped[]` にスキップ理由が入る。
- `dailyMetrics[]` に `stepCount`、`sleepMinutes`、`deepSleepMinutes`、`hrvMs`、`rhrBpm` の日次正規化結果が入る。
- `stepsResult.source` は、`reconcile` 成功時は `reconcile`、`dailyRollUp` 成功時は `dailyRollUp`、フォールバック成功時は `listFallback` になる。
