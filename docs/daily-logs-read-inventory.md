...
// SELECT MAX(updated_at) FROM daily_logs  ← stale 判定の分離
```

TDEE ページでは:
- KPI/テーブル: `fetchRecentDailyLogs(14)`
- stale 判定: `fetchLatestUpdatedAt()`
- グラフ: enriched_logs 主軸（rawLogs fallback は表示品質トレードオフを受け入れ）

### 優先度 Medium — Macro 軽量化（#166）
...
- `fetchLatestUpdatedAt()` 共通クエリを先行実装するかどうかを判断する起点にする

### #166 — Macro 軽量化

- `fetchMacroDailyLogs(60)` を新規追加（SELECT 6列・DESC LIMIT 60）
- stale 判定を `fetchLatestUpdatedAt()` に分離
