...
バッチごとに必要な列・期間が異なるため、front SSR の projection 最適化とは別に扱う。
...
**使用クエリ:** `fetchMacroDailyLogs(60)` (6列・DESC LIMIT 60 — #166 実装済み)
...
> クエリを 60 日に絞っても KPI 計算（直近 30 日以内）はカバーされる。

---

### 2-3. TDEE (`src/app/tdee/page.tsx`)

**使用クエリ:** `fetchTdeeDailyLogs(180)` + `fetchLatestUpdatedAt()` (3列・DESC LIMIT 180 — #166 実装済み)
...
【full read（ML バッチ / enrich.py / predict.py）】
