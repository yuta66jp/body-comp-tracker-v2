            ...
            {stats.map((s) => {
              ...
              return (
                <tr key={s.month} className="transition-colors hover:bg-slate-50/70">
                  {/* 月 */}
                  <td className="py-2.5 pr-3 font-mono font-medium text-slate-600 whitespace-nowrap">
                    {s.month}
                  </td>
                      ...
                      <span className="flex flex-wrap gap-x-2 gap-y-0.5">
                        {trainingEntries.map(({ type, count }) => (
                          <span key={type} className="whitespace-nowrap tabular-nums">
                            ...
                            <span className="ml-0.5 font-semibold text-slate-600">{count}</span>
                          </span>
                        ))}
                      </span>
                    ) : (
                      ...
                      <span className="flex flex-wrap gap-x-2 gap-y-0.5">
                        {flagEntries.map((k) => (
                          <span key={k} className="whitespace-nowrap tabular-nums">
                            ...
                            <span className="ml-0.5 font-semibold text-slate-600">
                              {s.flagCounts[k]}
                            </span>
                          </span>
                        ))}
