...
export type SaveDailyLogInput = {
  ...
  // ── Phase 2.5 追加 ──
  /**
   * @deprecated sleep_sessions テーブルが睡眠の source of truth (#515)。
   * CSV インポートの後方互換性のためフィールドは残存する。
   * 通常保存では使用しない。sleep_hours の更新は DB トリガーが担う。
   */
  ...
  /** 体重測定時刻 "HH:MM" 形式。null = 明示クリア。空腹時間算出専用。 */
  weigh_in_time?: string | null;
  // ── #436 追加 ──
  /** Apple Health 歩数（日次集計）。null = 明示クリア */
  step_count?: number | null;
  // bed_time は #515 で削除。睡眠は sleep_sessions テーブル経由で保存する (saveSleepSession)。
};
