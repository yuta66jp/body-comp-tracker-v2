...
`test_predict.py`, `test_enrich.py`, `test_backtest.py`.
To run Python tests locally: `cd ml-pipeline && pytest test_predict.py test_enrich.py test_backtest.py -v`
Dependencies: `pip install -r ml-pipeline/requirements-ci.txt` (pandas / scikit-learn / pytest — no torch required).
